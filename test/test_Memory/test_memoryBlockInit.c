...
void test_memoryBlockInit_given_write_a_data_to_a_address_is_within_ram_address_range(void){ 
  memoryBlockInit(&ramBlock, ramMemory, RAM_START_ADDR, RAM_SIZE);
...
void test_memoryBlockInit_given_write_a_data_to_a_address_is_without_ram_address_range(void){
  ErrorObject* err;
  memoryBlockInit(&ramBlock, ramMemory, RAM_START_ADDR, RAM_SIZE);
...
void test_memoryBlockInit_given_read_1_byte_from_a_address_is_without_ram_address_range(void){
  ErrorObject* err;
  memoryBlockInit(&ramBlock, ramMemory, RAM_START_ADDR, RAM_SIZE);
...
void test_memoryBlockInit_given_read_2_byte_from_a_address_is_without_ram_address_range(void){
  ErrorObject* err;
  memoryBlockInit(&ramBlock, ramMemory, RAM_START_ADDR, RAM_SIZE);
